...
def main():
    ...
    ax.set_xlabel(f"Tokens — {r1_title}", fontsize=13)
    ax.set_ylabel(f"Tokens — {r2_title}", fontsize=13)
    subtitle = "by attempt" if args.aggregate_by_attempts else "per problem, best retry"
    ax.set_title(f"Token usage comparison ({subtitle})", fontsize=15)
    ax.tick_params(axis="both", labelsize=11)
    ax.hlines(16384, xmin=lo, xmax=hi, linestyle="--", linewidth=3, color="blue", label="TIR turn token limit")
    ax.vlines(40960, ymin=lo, ymax=hi, linestyle="--", linewidth=3, color="red", label="Goedel context limit")
